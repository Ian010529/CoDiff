...
class BaseModel(LightningModule):
    ...
    def on_training_epoch_end(self):
        ...
        # 删除了传入的 outputs，因为在 on_training_epoch_end 中 outputs 是 None
        # 现在直接返回 None
        return self.allsplit_epoch_end("train", None)
    ...
    def on_validation_epoch_end(self):
        ...
        # 删除了传入的 outputs，因为在 on_validation_epoch_end 中 outputs 是 None
        # 现在直接返回 None
        return self.allsplit_epoch_end("val", None)
    ...
    def on_test_epoch_end(self):
        # 2. 从我们自己创建的列表中获取 outputs
        outputs = self.test_step_outputs
        # 3. 执行原始的逻辑
        self.save_npy(outputs)
        self.cfg.TEST.REP_I = self.cfg.TEST.REP_I + 1
        
        # 4. 执行 epoch_end 的剩余逻辑
        epoch_end_results = self.allsplit_epoch_end("test", outputs)

        # 5. 清空列表，为下一次测试做准备
        self.test_step_outputs.clear()

        return epoch_end_results
