...
# from mld.models.operator.freq_refiner import Frequency_Domain1d


# LatentSelfAttention
from mld.models.operator.LatentSelfAttention import LatentSelfAttention
...
class MLD(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        # # frequency domain refiner
        # self.freq_refiner = Frequency_Domain1d(channels=self.nfeats) 

        # LatentSelfAttention
        # 增加一个可选的配置项，用于控制是否启用潜在空间增强器
        self.use_latent_enhancer = self.cfg.model.get('use_latent_enhancer', False)
        if self.use_latent_enhancer:
            latent_dim = self.latent_dim[1]
            # num_heads 可以做成可配置的，这里先用默认值 4
            num_heads = self.cfg.model.get('latent_enhancer_heads', 4) 
            self.latent_enhancer = LatentSelfAttention(latent_dim=latent_dim, num_heads=num_heads)
            print("INFO: MLD model initialized with LatentSelfAttention enhancer.")
        # ------------------------------------
    ...
    def train_diffusion_forward(self, batch):
        ...
        # ##################################################################
        # 如果增强器在初始化时被创建了，就使用它
        if hasattr(self, 'latent_enhancer'):
            z = self.latent_enhancer(z)
        # ##################################################################
