...
import shutil
import sys
sys.path.append("/home3/tbkv79/motion-latent-diffusion/blender-2.93.18-linux-x64/2.93/python/lib/python3.9/site-packages")
...
def render_cli() -> None:
    ...
    print(f"DEBUG: 从配置文件中加载的 FPS 值为: {cfg.RENDER.FPS}") 
    ...
    for path in paths:
        ...
        if cfg.RENDER.MODE == "video":
            if cfg.RENDER.DOWNSAMPLE:
                print(f"--- 步骤 2 (render.py): 准备创建 Video 对象, 传递的 fps 值为: {cfg.RENDER.FPS}")
