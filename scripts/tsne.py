...
# 计算每一步 diffusion 的 silhouette score    
def compute_silhouette_over_steps(latents, classids, nsample=30, metric="cosine"):
    """
    计算每一步 diffusion 的 silhouette score
    latents: [classes, samples, steps, latent_dim]
    """
    nsteps = latents.shape[2]
    scores = []
    ...
    for step in range(nsteps):
        ...
        for idx, cid in enumerate(classids):
            z = latents[cid, :nsample, step, :]  # [nsample, latent_dim]
            feats.append(z)
            labels += [idx] * nsample
        ...
        try:
            score = silhouette_score(feats, labels, metric=metric)
        except Exception:
            score = np.nan
        scores.append(score)
...
# 绘制 silhouette score 曲线
def plot_silhouette_curve(scores, output_dir):
    outpath = pjoin(output_dir, "silhouette_curve.png")
    plt.figure(figsize=(6, 4))
    plt.plot(range(len(scores)), scores, marker="o")
    plt.xlabel("Diffusion Step")
    plt.ylabel("Silhouette Score (cosine)")
    plt.title("Silhouette score over diffusion steps")
    plt.grid(True)
    plt.savefig(outpath, bbox_inches="tight")
    print(f"[INFO] Silhouette curve saved at {outpath}")
    plt.close()
...
def main():
    ...
    # Compute Silhouette curve
    scores = compute_silhouette_over_steps(latents, classids=[8, 6, 5])
    np.save(pjoin(output_dir, "silhouette_scores.npy"), np.array(scores))
    plot_silhouette_curve(scores, output_dir)

    with open(pjoin(output_dir, "silhouette.txt"), "w") as f:
        for i, s in enumerate(scores):
            f.write(f"Step {i}: {s:.4f}\n")
    print(f"[INFO] Silhouette scores saved at {pjoin(output_dir, 'silhouette.txt')}")
