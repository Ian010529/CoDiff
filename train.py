...
# # CrossAttentionFusion1D
# # =========================================================================================
# # 1. 在文件顶部，正确地从我们自己的模块文件中导入
# import types
# from mld.models.operator.CrossAttentionFusion1D import CrossAttentionFusion1D 
...
# # =========================================================================================
# # 1. 导入我们需要的模块
# from mld.models.operator.FRFN1D import FRFN1D  # 导入我们自己的模块
# from mld.models.operator.cross_attention import TransformerEncoderLayer
# import types
# # =========================================================================================
...
# from mld.models.operator.LRSA1D import LRSA1D 
# from mld.models.operator.cross_attention import TransformerEncoderLayer
# # =========================================================================================

# =========================================================================================
# ############# LatentSelfAttention 动态缝合代码块 #############
# 1. 在 train.py 文件顶部，导入我们新创建的模块
import types
...
def main():
    ...
    if cfg.TRAIN.RESUME:
        ...
        backcfg = cfg.TRAIN.copy() # backup train config
        backlogcfg = cfg.LOGGER.copy() # backup logger config
        if os.path.exists(resume):
            file_list = sorted(os.listdir(resume), reverse=True)
            for item in file_list:
                if item.endswith(".yaml"):
                    cfg = OmegaConf.load(os.path.join(resume, item))
                    cfg.TRAIN = backcfg # restore train config
                    cfg.LOGGER = backlogcfg # restore logger config
                    break
